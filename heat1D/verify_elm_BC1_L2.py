...
class Net_Residual_BC1(Net):
    def forward(self, t_input):
        ...
        model_output = torch.matmul(t_input, self.hidden.weight[:,0:1].t()) + self.hidden.bias[:,None].t() + self.hidden.weight[:,1:2].t()
...
if __name__ == "__main__":
    ...
    for ti in range(num_samples):
        ...
        with open("verification_elm_1d_IC_BC_heat_L2.log", "a") as f:
            f.write(f"t: {t_test[ti].item():.3f}, x: {x_max}, Min lb: {max(min(lb1_list),min(lb2_list))}, Max ub: {total_ub}, Time: {time() - start_time:.2f}\n")
