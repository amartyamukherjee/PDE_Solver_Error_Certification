...
class Net_Residual_IC(Net):
    def forward(self, x_input):
        ...
        return model_output - torch.sin(x_input*torch.pi)
...
if __name__ == "__main__":
    ...
    for xi in range(num_samples):
        ...
        total_ub += max(ub1.max().item(),lb1.min().abs().item())/(num_samples)
        # ub2_list.append(ub2.max().item())
        ub2_list.append(1)
        
        with open("verification_elm_1d_IC_BC_heat_L2.log", "a") as f:
            f.write(f"t: {t_min:.3f}, x: {x_test[xi].item():.3f}, Min lb: {max(min(lb1_list),min(lb2_list))}, Max ub: {total_ub}, Time: {time() - start_time:.2f}\n")
