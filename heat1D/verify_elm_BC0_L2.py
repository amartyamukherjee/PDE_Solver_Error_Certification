...
class Net_Residual_BC0(Net):
    def forward(self, t_input):
        # model_output = self.beta(super().forward(x_input))
        model_output = torch.matmul(t_input, self.hidden.weight[:,0:1].t()) + self.hidden.bias[:,None].t()
        model_output = activation(model_output)
        model_output = torch.matmul(model_output, self.beta.weight.t())
        return model_output - 0

if __name__ == "__main__":
    net_residual_bc0 = Net_Residual_BC0(INPUT_DIM, HIDDEN_DIM)
    net_residual_bc0 = net_residual_bc0.to(device)#.double()
    ...
    if os.path.exists(model_path):
        ...
        net_residual_bc0.load_state_dict(model_state["net_state_dict"]) # Load state for 'net_residual_bc0' as well
        net_residual_bc0.eval() # Set to evaluation mode

    samples = (torch.rand((N_SAMPLES, 1), dtype=torch.float32) * 2 - 1).to(device)

    print(net_residual_bc0(samples).shape) # Pass 'samples' to the forward method
    ...
    for ti in range(num_samples):
        ...
        lb1, ub1, lb2, ub2 = verification1D(net_residual_bc0,t_test[ti],t_test[ti+1], False, device)
        lb1_list.append(lb1.min().item())
        # lb2_list.append(lb2.min().item())
        lb2_list.append(-1)
        ub1_list.append(ub1.max().item())
        total_ub += torch.exp(-t_test[ti])*max(ub1.max().item(),lb1.min().abs().item())**2/(num_samples)
        # ub2_list.append(ub2.max().item())
        ub2_list.append(1)
        
        with open("verification_elm_1d_IC_BC_heat_L2.log", "a") as f:
            f.write(f"t: {t_test[ti].item():.3f}, x: {x_min}, Min lb: {max(min(lb1_list),min(lb2_list))}, Max ub: {total_ub}, Time: {time() - start_time:.2f}\n")
