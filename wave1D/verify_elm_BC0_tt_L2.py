...
class Net_Residual_BC0_t(Net):
    def forward(self, t_input):
        ...
        hprime = activation_double_prime(model_output)
        dt = (hprime * Wt**2) @ beta.t()          # (N,1)
        return dt

if __name__ == "__main__":
    net_residual_bc0 = Net_Residual_BC0_t(INPUT_DIM, HIDDEN_DIM)
    ...
    for ti in range(num_samples):
        ...
        with open("verification_elm_1d_BC_dt2_wave_L2.log", "a") as f:
            f.write(f"t: {t_test[ti].item():.3f}, x: {x_min}, (dt^2), Min lb: {max(min(lb1_list),min(lb2_list))}, Max ub: {total_ub}, Time: {time() - start_time:.2f}\n")
