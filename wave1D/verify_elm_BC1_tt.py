...
if __name__ == "__main__":
    ...
    for ti in range(num_samples):
        ...
        with open("verification_elm_1d_BC_dt2_wave.log", "a") as f:
            f.write(f"t: {t_test[ti].item():.3f}, x: {x_max}, (dt^2), Min lb: {max(min(lb1_list),min(lb2_list))}, Max ub: {min(max(ub1_list),max(ub2_list))}, Time: {time() - start_time:.2f}\n")
