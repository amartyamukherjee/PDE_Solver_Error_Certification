...
if __name__ == "__main__":
    ...
    for xi in range(num_samples):
        ...
        with open("verification_elm_1d_IC_dt_wave.log", "a") as f:
            f.write(f"t: {t_min:.3f}, x: {x_test[xi].item():.3f} (Neumann), Min lb: {max(min(lb1_list),min(lb2_list))}, Max ub: {min(max(ub1_list),max(ub2_list))}, Time: {time() - start_time:.2f}\n")
