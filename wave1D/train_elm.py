...
c = 0.2
n = 1
w = math.sqrt((alpha**2)*(n*math.pi)**2+c)
...
def true_solution(var):
    t = var[..., 0]
    x = var[..., 1]
    return torch.cos(w*t) * torch.sin(x*torch.pi)
...
def compute_residuals(net, samples):
    ...
    residuals = H_tt_matrix - (alpha**2) * H_xx_matrix + c * H

    b = torch.zeros(N, 1).to(device)
    return residuals, b
...
def compute_boundary(net, samples):
    ...
    H = net(x)
    b = true_solution(x).unsqueeze(1).to(device)
    # print(b.shape)
    return H, b
...
if __name__ == "__main__":
    ...
    A4, b4 = compute_neumann(net, ic_points)
    # print("A1:", A1.shape)
    # print("b1:", b1.shape)
