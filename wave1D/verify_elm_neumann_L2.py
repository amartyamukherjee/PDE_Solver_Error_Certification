...
class Net_Residual_IC(Net):
    def forward(self, x_input):
        ...
        linear_output = torch.matmul(x_input, self.hidden.weight[:,1:2].t()) + self.hidden.bias[:,None].t()
        H_t_matrix = activation_prime(linear_output) * self.hidden.weight[:, 0].unsqueeze(0)
        model_output = torch.matmul(H_t_matrix, self.beta.weight.t())
...
if __name__ == "__main__":
    ...
    for xi in range(num_samples):
        ...
        with open("verification_elm_1d_IC_dt_wave_L2.log", "a") as f:
            f.write(f"t: {t_min:.3f}, x: {x_test[xi].item():.3f} (Neumann), Min lb: {max(min(lb1_list),min(lb2_list))}, Max ub: {total_ub}, Time: {time() - start_time:.2f}\n")
