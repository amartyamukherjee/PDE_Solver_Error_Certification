...
from train_elm import Net, INPUT_DIM, HIDDEN_DIM, N_SAMPLES, alpha, c, activation, activation_prime, activation_double_prime, x_max, x_min, t_max, t_min
...
class Net_PDE_Residual(Net):
    def forward(self, x_input): # Renamed to avoid confusion with global 'x'
        ...
        residuals = H_tt_matrix - (alpha**2) * H_xx_matrix + c * activation(linear_output)
...
if __name__ == "__main__":
    ...
    for ti in range(num_samples):
        for xi in range(num_samples):
            ...
            with open("verification_elm_1d_wave.log", "a") as f:
