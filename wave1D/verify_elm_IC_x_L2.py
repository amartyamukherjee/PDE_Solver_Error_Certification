...
class Net_Residual_IC(Net):
    def forward(self, x_input):
        ...
        model_output = torch.matmul(x_input, Wt2) + b
        hprime = activation_prime(model_output)
        dt = (hprime * Wt2) @ beta.t() - torch.pi * torch.cos(x_input*torch.pi)
        return dt
...
if __name__ == "__main__":
    ...
    for xi in range(num_samples):
        ...
        with open("verification_elm_1d_IC_dx_wave_L2.log", "a") as f:
            f.write(f"t: {t_min:.3f}, x: {x_test[xi].item():.3f}, (dx) Min lb: {max(min(lb1_list),min(lb2_list))}, Max ub: {total_ub}, Time: {time() - start_time:.2f}\n")
