...
from train_elm import Net, INPUT_DIM, HIDDEN_DIM, N_SAMPLES, activation, activation_prime, activation_double_prime, source_term
...
class Net_PDE_Residual(Net):
    def forward(self, x_input): # Renamed to avoid confusion with global 'x'
        ...
        return torch.matmul(residuals, self.beta.weight.t()) - source_term(x_input).unsqueeze(1)
...
if __name__ == "__main__":
    ...
    print(net_residual(samples).abs().max().item()) # Pass 'samples' to the forward method
    ...
    for ti in range(num_samples):
        for xi in range(num_samples):
            ...
            with open("verification_elm_2d_poisson.log", "a") as f:
                f.write(f"t: {t_test[ti].item():.3f}, x: {x_test[xi].item():.3f}, Min lb: {max(min(lb1_list),min(lb2_list))}, Max ub: {min(max(ub1_list),max(ub2_list))}, Time: {time() - start_time:.2f}\n")
