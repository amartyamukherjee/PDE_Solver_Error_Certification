...
class Net_Residual_IC(Net):
    def forward(self, x_input):
        ...
        model_output = torch.matmul(x_input, self.hidden.weight[:,1:2].t()) + self.hidden.bias[:,None].t() + self.hidden.weight[:,0:1].t()
...
if __name__ == "__main__":
    ...
    for xi in range(num_samples):
        ...
        with open("verification_elm_1d_IC_BC_wave.log", "a") as f:
            f.write(f"t: {t_max:.3f}, x: {x_test[xi].item():.3f}, Min lb: {max(min(lb1_list),min(lb2_list))}, Max ub: {min(max(ub1_list),max(ub2_list))}, Time: {time() - start_time:.2f}\n")
